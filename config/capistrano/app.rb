...
namespace :app do
  ...
  task :symlinks do
    ...
    run "ln -nfs #{shared_path}/bin/web_server #{release_path}/bin/web"
    run "ln -nfs #{shared_path}/config/Gemfile.lock #{release_path}/Gemfile.lock"
    run "ln -nfs #{shared_path}/config/database.yml #{release_path}/config/database.yml"

    # folders
    run "ln -nfs #{shared_path}/uploads #{release_path}/public/uploads"
    run "ln -nfs #{shared_path}/video #{release_path}/public/video"
  end
  ...
  task :configs do
    ...
    template("web_server.rb", "#{dir_bin}/web_server")
    # template("database.production.yml", "#{dir_conf}/database.yml")
    run "chmod -R 744 #{shared_path}/bin/*"
  end
